...
> 强缓存是指浏览器在请求资源时，通过判断缓存是否过期来决定是否使用缓存的一种方式。

当浏览器第一次请求资源时，服务器返回的响应头中会包含 `Cache-Control`
或 `Expires` 字段，用于控制资源的缓存。浏览器在接收到资源后，会将其保存到本地，并在下一次请求该资源时，先判断本地缓存是否过期，如果未过期，则直接从本地缓存读取资源，不再向服务器发起请求。
下面是强缓存的实现方式。
...
> 协商缓存是浏览器缓存机制的一种实现方式，用于在强缓存失效时向服务器发起请求验证资源是否发生变化，以决定是否使用缓存的一种方式。

协商缓存主要包括两个部分：请求头和响应头。

当浏览器判断本地缓存已经过期时，会向服务器发起请求，服务器返回的响应头中会包含 `Last-Modified` 或 `ETag`字段，用于标识资源的最后修改时间和唯一标识。
浏览器在接收到资源后，会将其保存到本地，并在下一次请求该资源时，先向服务器发起请求，请求头中包含上一次响应头中返回的 `Last-Modified`或 `ETag` 值，
服务器会根据这些值判断该资源是否有更新，如果没有更新，则返回 `304` 状态码，并告诉浏览器可以从本地缓存中获取该资源；如果有更新，则返回新的资源内容和响应头，并将其保存到本地缓存。
...
的生成会消耗一定地计算资源，因此在实际应用中需要考虑性能开销。
