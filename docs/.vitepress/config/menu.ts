...
const getNavItem = (path: string): DefaultTheme.NavItem[] => sync(
    ...
    const link = path.replace(/^docs/, '').replace('index.md', '')
    groups.push({
        text: data.title,
        link: link,
        activeMatch: link
...
export const getNav = (): DefaultTheme.NavItem[] => [
    ...
    ...getNavItem('docs').sort((a, b) => navOrder.indexOf((a as DefaultTheme.NavItemWithLink).link) - navOrder.indexOf((b as DefaultTheme.NavItemWithLink).link))
]
