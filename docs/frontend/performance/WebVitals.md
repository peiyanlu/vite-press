...
| 指标名                                | 含义                  | 衡量点                    | 理想值     |
|------------------------------------|---------------------|------------------------|---------|
| **LCP（Largest Contentful Paint）**  | 最大内容绘制时间            | 页面最大内容元素出现在视口中所需时间     | ≤ 2.5s  |
| **FID（First Input Delay）**         | 首次输入延迟              | 用户首次交互（点击等）到浏览器响应之间的延迟 | ≤ 100ms |
| **INP（Interaction to Next Paint）** | 与用户交互后的响应速度（取代 FID） | 所有交互中响应最慢的一次           | ≤ 200ms |
| **CLS（Cumulative Layout Shift）**   | 累计布局偏移              | 页面布局在加载过程中的跳动程度        | ≤ 0.1   |
